import { BarChart3, FileText, LayoutDashboard, MessageSquare, Newspaper, Settings, ShieldCheck, Tag, Users } from 'lucide-react';
...
export default function AdminLayout() {
  ...
  return (
    <div className="min-h-screen bg-gray-100 lg:grid lg:grid-cols-[260px_1fr]">
      <aside className="border-r border-gray-200 bg-gray-950 text-white">
        <div className="flex h-16 items-center gap-2 border-b border-white/10 px-5">
          <ShieldCheck className="text-brand-red" />
          <span className="font-extrabold">FTP Admin</span>
        </div>
        <nav className="flex gap-1 overflow-x-auto p-3 lg:grid lg:overflow-visible">
          {visibleLinks.map(([label, path, Icon]) => (
            <NavLink
              ...
              to={path}
              className={({ isActive }) =>
                `flex shrink-0 items-center gap-3 rounded-md px-3 py-2 text-sm font-semibold ${isActive ? 'bg-white text-gray-950' : 'text-gray-300 hover:bg-white/10 hover:text-white'}`
              }
            >
              <Icon size={18} /> {label}
            </NavLink>
